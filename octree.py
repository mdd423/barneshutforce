...
class octree(object):
    ...
    # Here is the defined acceleration formula
    # Swap this with boid acceleration
    # Maybe still want acceleration so they will orbit objects
    # 
...
class boidnode(object):
    def __init__(self, tree, particles, masses, boidness, vel):
        self.tree = tree
        self.com = self.center
        self.children = [] #list of children of this node
        ...
        self.cob = self.center
        self.boidness = boidness
        self.B = np.sum(self.boidness)
        self.vel = vel
    ...
    def update_cob(self):
        ...
        """
        self.cob = np.zeros(3)
        self.cob[0] = np.dot(self.vel[:,0], self.boidness[:])/self.B
        self.cob[1] = np.dot(self.vel[:,1], self.boidness[:])/self.B
        self.cob[2] = np.dot(self.vel[:,2], self.boidness[:])/self.B
...
class kdboidoctree(octree):
    ...
    Attributes:
        ...
        particles_dict: dictionary mapping particle index to the leaf node the particle belongs to (dictionary)
    """
    def __init__(self, particles, masses, vel, boidness, leafsize):
        self.particles = particles
        self.masses = masses
    ...
    def create_tree(self):
        tree = scipy.spatial.KDTree(self.particles,self.leafsize)
        root = boidnode(tree, self.particles, self.masses, self.boidness, self.vel)
        # root = node(tree, self.particles, self.masses)
    ...
    def tranverse(self,n0,n1,theta,idx,ret,G,eps=0.01):
        """
        given two nodes n0 and n1, and some tol theta, traverse the tree till it's far enough that you can approximate the
        node as a "particle" and add the gravitational acceleration of that particle to the ret array. n1 is the leaf node that 
        holds the particle we are calculating the accel for.
        ...
        if(n0 == n1):
            return
        dr = n0.com - n1.com
        vcmag = numpy.linalg.norm(np.cross(n0.cob,n1.cob))
        vavg = (n0.cob + n1.cob)/2
        r2 = np.sum(dr**2)
        r = np.sqrt(r2)
        size_of_node = n0.box.xhigh - n0.box.xlow
        if(size_of_node/r < theta or n0.leaf):
            ret += vcmag * vavg / (r2 +  + eps**2)
        else:
            for c in n0.children:
                self.traverse(c, n1, theta, idx, ret, G)
        return ret